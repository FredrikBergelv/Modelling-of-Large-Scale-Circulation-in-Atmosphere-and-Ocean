\section{Theory}
\textbf{i)} \textbf{In order to derive Sverdrup gyre circulation, one needs the following assumptions: One needs a set of linear and stationary shallow water equation, one needs a wind stress, a beta plane and an inviscist fluid on a flat bottom. }
\begin{enumerate}
...
This gives an interesting shape of the gyre, where we see a net southward velocity in the latitudinal center of the domain, and less flux away from the center. Observing \autoref{eq:theory}, one can see that this yields positive streamfunction values. We will thus see a "parabolic-like" structure along the longitudinal direction, with the peak of the parabola in the east. We thus see zero flux close to the eastern boundary. 
\textbf{The gyre decreases towards the east, stream lines are not closed on the western boundary}

\textbf{v)} In traditional Munk theory we also add a Laplace friction term to the shallow water equations. We thus assume that we are given the set of shallow water equations as 
\begin{subequations}
    \label{eq:SWE}
...
\end{enumerate}
\textbf{Diffusive drition/vicosity (laplacian), much lager in x than y direction Boundary conditions $\Psi(x_\text{East})=\Psi(x_\text{West})=0$}
...
\textbf{Closed streamlines on wesr, but sverdrup in interior and east}
...
\end{equation}
where $\Psi_\text{Sverdrup}$ is the Sverdrup solution and $\delta_M$ is the Munk layer defined as 
\begin{equation}
...
\textbf{viii)} One can approximate the wind stress in the North Atlantic with 
\begin{equation}
...
\textbf{Half wavelength of sin with wind = 0 in center westerlies and tradewinds. }