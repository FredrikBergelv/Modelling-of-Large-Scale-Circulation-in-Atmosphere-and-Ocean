...
def plot_streamfunction(folder, times, show=False, u10=False, A=False, Lx=Lx):
    ...
    for i in range(n):
        ...
        # --------------------------------------------------
        ax_psi.plot(x_plot, psi_profiles[i], linewidth=2, color="C0")
        ...
        # --------------------------------------------------
        ax_v.plot(x_plot, v_profiles[i], linewidth=2, color="C1")
        ax_v.set_ylim(v_min, v_max)
        ax_v.grid(alpha=0.3)
        
        ax_v.set_xlabel(r"Latitude, $x/Lx$ [-]", fontsize=11)
        ax_psi.set_xlabel(r"Latitude, $x/Lx$ [-]", fontsize=11)
        ax_contour.set_xlabel(r"Latitude, $x/Lx$ [-]", fontsize=11)
