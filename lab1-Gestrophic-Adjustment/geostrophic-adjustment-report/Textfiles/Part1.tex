...
\begin{figure}[H]
    \centering    \includegraphics[width=0.6\textwidth]{Figures/sketch.pdf}
...
\begin{equation}
    ...
    \label{eq:solv}
\end{equation}
Note that the solutions are uniform in $y$, and will thus not depend on the variable.
...
Finally the initial perturbation of $\eta_0$ will also affect the solution, as the limit when $x\rightarrow\pm\infty$ is approached. Consequently, this will equally scale the meridional velocity $v$ since $v\propto\eta_0$. However $v$ is also directly dependent as $v\propto\tfrac{1}{fR}$, which means that $v\propto\tfrac{1}{\sqrt{H}}$ but not dependent on $f$. Thus, varying the Coriolis parameter should vary the spread of the meridional velocity, but not the max speed. Furthermore, varying the depth will also vary the spread, but also the the size of the max speed $\abs{v}$.  