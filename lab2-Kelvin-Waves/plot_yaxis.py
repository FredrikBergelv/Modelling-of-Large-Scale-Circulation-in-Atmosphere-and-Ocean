...
for j, (dataset,H,name) in enumerate(cases):
    ...
    for i, t in enumerate(times):
        ...
        if j==0 or j==2:
            ...
            h_theo = theo(yh_theo, c, t, Lx, Ly)
            ax.plot(yh_theo, h_theo, c=f"C4", label=fr"$\eta_{{\text{{theo.}}}}\left(x_\text{{p.}},y\right)$", linestyle="--", alpha=0.8)
            
        ax.plot(yh, ht, c=f"C{j}", label=fr"$\eta_{{\text{{{name}}}}}\left(x_\text{{p.}},y\right)$")
