...
PROGRAM ST_VENANT
   ...
   LOGICAL :: l_coast = .TRUE.            ! Should we use solid bc as coast?
   ...
   REAL    :: dx, dy, dt, ra, x, x0, y, shiftx, shifty, alp_t, tau_w
   ...
   & l_write_uv, l_solidbc, l_coast, l_periodic, l_up, l_vp, l_spongebc, S, btype, &
   ...
   ! Here we use "namelist" if nothing else is specified
   ! Else we use an namelist of our choice
   ...
   IF (.NOT. lexist) THEN
       PRINT *, 'ERROR: namelist file not found!'
       STOP
   END IF
   OPEN(UNIT=11, FILE=TRIM(namelist_file), FORM='FORMATTED', STATUS='OLD')
   PRINT *, 'Using namelist:', TRIM(namelist_file)
   ...
   alp_t = depth_diff*(2*D/Ly)

   tau_w = 1.225*1.5*0.001*(u10**2)
   if (l_wind) then
      PRINT *, 'Using tau_w:', tau_w
   endif
   ...
   IF (l_betaplane .OR. l_topography .OR. l_wind) THEN
...
IF (l_gaussian) THEN
      ...
      !* Shift gaussian
      IF (l_betaplane .OR. l_topography) THEN
         shiftx = 0.5*Lx
         shifty = 0
      ELSE
         shiftx = 0.5*Lx
   ...
   IF (l_coriolis) THEN
      ...
      V_u(1:Nx - 1, :) = ( &
                         v_tmp(1:Nx - 1, 1:Ny, -1) + &
                         v_tmp(1:Nx - 1, 0:Ny - 1, -1) + &
                         v_tmp(2:Nx, 1:Ny, -1) + v_tmp(2:Nx, 0:Ny - 1, -1) &
                         )/4.
      ! Define U_v, which is u in a v point
      U_v(:, 1:Ny - 1) = ( &
                         u_tmp(1:Nx, 1:Ny - 1, -1) + u_tmp(1:Nx, 2:Ny, -1) + &
                         u_tmp(0:Nx - 1, 1:Ny - 1, -1) + &
                         u_tmp(0:Nx - 1, 2:Ny, -1) &
                         )/4.
      ...
      V_u(0, :) = 2.*V_u(1, :) - V_u(2, :)
      V_u(Nx, :) = 2.*V_u(Nx - 1, :) - V_u(Nx - 2, :)
      U_v(:, 0) = 2.*U_v(:, 1) - U_v(:, 2)
      U_v(:, Ny) = 2.*U_v(:, Ny - 1) - U_v(:, Ny - 2)

      ! Periodic boundary condition
      ! Consider for example the condition (l_up .and. l_periodic)
      IF (l_up .and. l_periodic) THEN
         V_u(0, :) = (v_tmp(1, 0:Ny - 1, -1) + v_tmp(1, 1:Ny, -1) + v_tmp(Nx, 0:Ny - 1, -1) + v_tmp(Nx, 1:Ny, -1))/4.
         V_u(Nx, :) = V_u(0, :)
      END IF
      IF (l_vp .and. l_periodic) THEN
         U_v(:, 0) = (u_tmp(0:Nx - 1, 1, -1) + u_tmp(1:Nx, 1, -1) + u_tmp(0:Nx - 1, Ny, -1) + u_tmp(1:Nx, Ny, -1))/4.
         U_v(:, Ny) = U_v(:, 0)
      END IF
   ...
   DO jt = 2, nt
      ...
      IF (l_coriolis) THEN
         ...
         ! Consider for example the condition (l_up .and. l_periodic)
         IF (l_up .and. l_periodic) THEN
            V_u(0, :) = (v_tmp(1, 0:Ny - 1, 0) + v_tmp(1, 1:Ny, 0) + v_tmp(Nx, 0:Ny - 1, 0) + v_tmp(Nx, 1:Ny, 0))/4.
            V_u(Nx, :) = V_u(0, :)
         END IF
         IF (l_vp .and. l_periodic) THEN
            U_v(:, 0) = (u_tmp(0:Nx - 1, 1, 0) + u_tmp(1:Nx, 1, 0) + u_tmp(0:Nx - 1, Ny, 0) + u_tmp(1:Nx, Ny, 0))/4.
            U_v(:, Ny) = U_v(:, 0)
         END IF
...
CONTAINS
   ...
   SUBROUTINE COMPUTE_ENERGY()
      ...
      REAL(4), DIMENSION(1 + sx:Nx - sx, 1 + sy:Ny - sy, nb_save) :: epij, ekij, etij
      ...
      ep = 0.5*rho*g*sum(sum( &
                         h(1 + sx:Nx - sx, 1 + sy:Ny - sy, :)**2, 1), 1)*dx*dy
      ek = 0.5*rho*D*sum(sum( &
                         0.5*u(1 + sx:Nx - sx, 1 + sy:Ny - sy, :)**2.+ &
                         0.5*u(0 + sx:Nx - 1 - sx, 1 + sy:Ny - sy, :)**2., &
                         1), 1)*dx*dy + &
           0.5*rho*D*sum(sum( &
                         0.5*v(1 + sx:Nx - sx, 1 + sy:Ny - sy, :)**2.+ &
                         0.5*v(1 + sx:Nx - sx, 0 + sy:Ny - 1 - sy, :)**2., &
                         1), 1)*dx*dy
      ...
      IF (l_energyij) THEN
         ...
         epij = 0.5*rho*g*h(1 + sx:Nx - sx, 1 + sy:Ny - sy, :)**2*dx*dy
         ekij = 0.5*rho*D*( &
                0.5*u(1 + sx:Nx - sx, 1 + sy:Ny - sy, :)**2.+ &
                0.5*u(0 + sx:Nx - 1 - sx, 1 + sy:Ny - sy, :)**2. &
                )*dx*dy + &
